...
var i=Math.floor(Math.random()*57);
var a=i;
...
function gotResult(error, results){
    if(error){
        ...
        console.log(results);
        if(a==results[0]){
            document.getElementById("label").innerHTML="It is "+results[0].label;
            utterThis=new SpeechSynthesisUtterance("I got it! It is "+results[0].label);
        }
        else{
            document.getElementById("label").innerHTML="Is it "+results[0].label+ "?";
            utterThis=new SpeechSynthesisUtterance("Is it "+results[0].label+"?");
        }
...
document.getElementById("hello").innerHTML=quick_draw_data_set[a];
}
